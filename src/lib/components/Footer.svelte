...
<footer class="w-full bg-foreground text-background">
    <div class="mx-auto grid w-full max-w-6xl gap-10 px-8 py-14 text-center md:grid-cols-3 md:text-left">
        <div>
            <img class="mx-auto h-12 object-contain brightness-0 invert md:mx-0" src="/keyclub_horizontal_black.png" alt="Key Club"/>
        ...
        <div>
            ...
            <ul class="mt-4 space-y-2">
                {#each nav as item (item.href)}
                    <li><a class="text-muted-foreground hover:text-primary transition-colors" href={item.href}>{item.label}</a></li>
                {/each}
            </ul>
        </div>
        ...
        <div>
            ...
            <a class="mt-4 flex items-center justify-center gap-2 text-muted-foreground hover:text-primary md:justify-start transition-colors"
            ...
            <ul class="mt-5 flex justify-center gap-5 md:justify-start">
                {#each socials as social (social.label)}
                    <li>
                        <Button href={social.href} target="_blank" rel="noopener" size="icon" class="text-muted-foreground hover:text-primary transition-colors" variant="icon">
    ...
    <div class="border-t border-border px-8 py-5 text-center text-sm text-muted-foreground">
