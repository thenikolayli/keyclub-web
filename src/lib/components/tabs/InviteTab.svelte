...
<div class="mx-auto max-w-lg">
  {#if status === "success"}
    <div
      class="flex animate-in fade-in slide-in-from-bottom-4 duration-500 flex-col items-center gap-6 rounded-2xl border border-stone-700 bg-stone-900 px-8 py-16 text-center shadow-lg"
    >
      <div
        class="flex size-20 items-center justify-center rounded-full bg-primary/10"
      >
        <PartyPopper class="size-10 text-primary" />
      </div>
      <div>
        <h3 class="font-bold-gothic text-2xl text-stone-100">Invite sent!</h3>
        <p class="mt-2 text-stone-400">Invite has been created successfully.</p>
        <p class="mt-1 text-xs text-stone-500">
          An email is on its way to <span class="font-medium text-stone-200"
            >{email}</span
          >.
        </p>
      </div>
  ...
  {:else}
    ...
    >
      ...
      >
        ...
        <div>
          <h3 class="font-bold-gothic text-xl text-stone-100">
            Send an Invite
          </h3>
          <p class="text-sm text-stone-400">Add someone new to the club</p>
        </div>
      </div>

      <div class="space-y-5 px-8 pb-6 pt-6">
        <div class="space-y-2">
          <label class="text-sm font-medium text-stone-300" for="email"
            >Email</label
          >
          <Input
            id="email"
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-2">
            <label class="text-sm font-medium text-stone-300" for="first-name"
              >First name</label
            >
            <Input id="first-name" bind:value={firstName} placeholder="Ada" />
          </div>
          <div class="space-y-2">
            <label class="text-sm font-medium text-stone-300" for="last-name"
              >Last name</label
            >
            <Input
              id="last-name"
        ...
        <div class="space-y-2">
          <label class="text-sm font-medium text-stone-300" for="role"
            >Role</label
          >
          <div class="relative">
            <select
            ...
            <svg
              class="text-stone-400 pointer-events-none absolute right-2.5 top-1/2 size-4 -translate-y-1/2"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
