...
>
  ...
  <Drawer.Root bind:open={drawerOpen} direction="right">
    <Drawer.Trigger
      class="absolute right-3 top-5 md:hidden"
      aria-label="Toggle menu"
    >
      <Icon icon="solar:hamburger-menu-linear" class="size-10" />
    </Drawer.Trigger>
    <Drawer.Content
      class="w-[72%] max-w-xs border-primary data-[vaul-drawer-direction=right]:border-l-4 bg-background p-6 text-foreground"
    >
          ...
          {#each links as link (link.href)}
            <li>
              <a
                ...
                >{link.label}
                <span
                  class="pointer-events-none absolute bottom-0.5 left-0 h-0.5 w-full {isActive(
                    link.href,
                  )
                    ? 'bg-primary'
                    : 'bg-transparent'}"
      ...
      <img
        class="mx-auto h-28 w-28 object-contain opacity-90"
        src="/faz.webp"
        alt="Henry M. Jackson Key Club"
      />
    </Drawer.Content>
  ...
  <nav class="hidden md:block">
    <ul class="flex items-center gap-7 text-lg lg:gap-10 lg:text-xl">
      {#each links as link (link.href)}
        <li>
          <a class="relative py-1" href={link.href}>
            {link.label}
            <span
              class="pointer-events-none absolute bottom-0.5 left-0 h-0.5 w-full {isActive(
                link.href,
