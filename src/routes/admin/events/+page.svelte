<script lang="ts">
  ...
  onMount(() => {
    document.title = "Beekeeper - Log Event";
  });
...
<section class="flex flex-col text-foreground">
  ...
  <section class="flex flex-wrap p-4 gap-4">
    <div class="flex w-full max-w-md flex-col gap-4">
      <form {...logEvent.enhance(async ({ submit }) => await submit())}>
        <Card.Root>
          <Card.Header>
          ...
          <Card.Content class="flex flex-col gap-2">
            <Input
              placeholder="Attendance doc URL"
              {...logEvent.fields.url.as("url")}
            />
            <Button
              type="submit"
              class="mt-2"
              variant="default"
              disabled={logEvent.pending > 0}
            >
        ...
        <Alert.Root>
          ...
          <Alert.Description>
            <span class="font-medium"
              >{logEvent.result.data.event.total_hours!.toFixed(2)} total hours</span
            > were written to the attendance doc, the Events sheet, and the EventsMembers
            sheet.
          </Alert.Description>
        </Alert.Root>
      ...
      <Card.Root>
        ...
        >
          ...
          <Badge variant="secondary">
            {logEvent.result?.ok
              ? logEvent.result.data.membersLogged.length
              : 0}
          </Badge>
        </Card.Header>
        <Card.Content>
    ...
    <div class="flex w-full max-w-4xl flex-1 min-w-[20rem] flex-col gap-4">
      <Card.Root>
        ...
        >
          ...
          <Badge variant="secondary">
            {logEvent.result?.ok
              ? logEvent.result.data.membersNotLogged.length
              : 0}
          </Badge>
        </Card.Header>
        <Card.Content>
      ...
      <Card.Root>
        ...
        <Card.Content>
          ...
          <ul class="list-disc list-inside mt-2">
            <li>
              Write each volunteer's calculated hours into the attendance doc
            </li>
            <li>
              Append the event (name, total hours, link) to the Events sheet
            </li>
            <li>Record each member's hours in the EventsMembers sheet</li>
          </ul>
          <br />
          Members whose names don't match a column in the EventsMembers sheet show
          up under "Members Not Logged" and won't have their hours recorded.
        </Card.Content>
      </Card.Root>
    </div>
  </section>
