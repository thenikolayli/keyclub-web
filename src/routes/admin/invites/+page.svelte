...
<section class="flex flex-col text-foreground">
  ...
  <section class="flex flex-wrap p-4 gap-4">
    <div class="flex w-full max-w-md flex-col gap-4">
      <form {...invite.enhance(async ({ submit }) => await submit())}>
        <Card.Root>
          ...
          <Card.Content class="flex flex-col gap-2">
            ...
            <Select.Root
              type="single"
              onValueChange={(value) => invite.fields.role.set(value)}
            >
              <Select.Trigger class="capitalize">
                {invite.fields.role.value() === undefined
                  ? "Role"
                  : invite.fields.role.value()}
              </Select.Trigger>
              <Select.Content>
      ...
      {#if invite.result && invite.result.ok}
        <Alert.Root>
          ...
          <Alert.Description>
            Please tell the user to check their email and finish creating their
            account.
          </Alert.Description>
        </Alert.Root>
    ...
    <div class="w-full max-w-4xl flex-1 min-w-[20rem]">
      <Card.Root>
        <Card.Header>
          <Card.Title>Roles and Permissions</Card.Title>
        </Card.Header>
        <Card.Content>
          <p>
            There are currently three roles available: <strong>member</strong>,
            <strong>leader</strong>, and <strong>officer</strong>.
          </p>
          <br />
          Members do not have access to any tools. Leaders have access only to the
          following tools:
