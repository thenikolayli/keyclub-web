...
<section class="flex flex-col text-foreground">
  ...
  <section class="flex flex-wrap p-4 gap-4">
        ...
        <Alert.Root>
          ...
          <Alert.Description>
            <a
              ...
              href={calendar.result.data.link}>{calendar.result.data.name}</a
            >
    ...
    <div class="w-full max-w-4xl flex-1 min-w-[20rem]">
      <Card.Root>
        <Card.Header>
          <Card.Title>Adding an event to the calendar</Card.Title>
        </Card.Header>
        <Card.Content>
          ...
          <ul class="list-disc list-inside">
            ...
            <li>
              The attendance doc is shared with the Google service account (the
              one with the long email)
            </li>
            <li>
              The fields in the first table of the attendance doc are filled out
              and formatted according to the
              <a
                class="underline text-secondary"
                href="https://docs.google.com/document/d/1r6Jz_XoWstzBPy_G2Oja_ggeVrzbK2s7qqLAVOalfPA/edit?usp=sharing"
                >event template</a
              >
            </li>
          </ul>
