...
<section class="w-full bg-background px-6 py-16 text-foreground">
  <div use:reveal class="mx-auto max-w-xl">
    <form
      class="rounded-2xl bg-background p-8 shadow-lg"
      {...getHours.enhance(async ({ submit }) => await submit())}
    >
    ...
    <div class="mt-8" aria-live="polite">
      ...
      {:else if getHours.result && getHours.result.ok}
        <div
          class="overflow-hidden rounded-2xl bg-foreground text-background shadow-lg"
        >
          <div class="bg-secondary px-6 py-5">
            <h2 class="font-bold-gothic text-3xl text-primary">
              {getHours.result.data.name}
            </h2>
            <p class="mt-1 text-muted-foreground">
              {getHours.result.data.class} Class of {getHours.result.data
                .grad_year}
            </p>
          </div>
          <div class="grid grid-cols-2 divide-x divide-border">
            <div class="p-6 text-center">
              ...
              <p class="font-bold-gothic mt-1 text-4xl text-primary">
                {getHours.result.data.all_hours}
              </p>
            </div>
            <div class="p-6 text-center">
              <p class="text-sm uppercase tracking-wide text-muted-foreground">
              ...
              <p class="font-bold-gothic mt-1 text-4xl text-primary">
                {getHours.result.data.term_hours}
              </p>
            </div>
          </div>
        </div>
